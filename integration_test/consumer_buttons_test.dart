...
void main() {
  ...
  Object? arguments(String route) {
    if (route == '/merchant-details') return <String, dynamic>{
      '_id': const String.fromEnvironment('QA_MERCHANT_ID'), 'storeName': 'QA Store',
    };
    if (route == '/deal-details') return <String, dynamic>{
      '_id': const String.fromEnvironment('QA_DEAL_ID'), 'title': 'QA Lunch Deal',
      'currentPrice': 15, 'originalPrice': 20, 'description': 'Synthetic UI test offer',
      'merchantId': const String.fromEnvironment('QA_MERCHANT_ID'),
    };
    if (route == '/checkout') return <String, dynamic>{
      'subtotal': 10.0, 'deliveryFee': 6.0, 'deliveryAddress': 'Test Street 1',
      'cartItems': <Map<String, dynamic>>[{
        'id': const String.fromEnvironment('QA_PRODUCT_ID'), 'name': 'QA Coffee',
  ...
  testWidgets('consumer buttons on the iOS simulator', (tester) async {
    ...
    Future<void> openRoute(String route) async {
      ...
      await settle(tester);
      Get.until((r) => r.settings.name == '/main-app' || r.isFirst);
      await settle(tester);
      if (Get.currentRoute != '/main-app') {
        Get.offAllNamed('/main-app');
        await settle(tester);
      }
      if (Get.isRegistered<MainAppController>()) {
        Get.find<MainAppController>().currentIndex.value = 0;
        await settle(tester);
      }
      if (route != '/main-app') {
    ...
    const excluded = {
      '/splash', '/main-app', '/home', '/q-r-scanner', '/order-tracking',
      '/verification', '/otp-verify', '/reset-password', '/success-account',
    };
    const requestedRoutes = String.fromEnvironment('QA_ROUTES');
    final routes = requestedRoutes.isNotEmpty
        ? requestedRoutes.split(',')
        : ['/main-app', ...AppPages.routes.map((p) => p.name)
